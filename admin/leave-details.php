...
<body>
    ...
    if (strlen($_SESSION['alogin']) == 0) {
        ...
        if (isset($_POST['update'])) {
            ...
            // Fetch leave details
            $sqlLeave = "SELECT FromDate, ToDate, empid FROM tblleaves WHERE id=:leaveid";
            $queryLeave = $dbh->prepare($sqlLeave);
            $queryLeave->bindParam(':leaveid', $did, PDO::PARAM_STR);
            $queryLeave->execute();
            $leaveDetails = $queryLeave->fetch(PDO::FETCH_OBJ);

            // Calculate number of leave days
            $fromDate = new DateTime($leaveDetails->FromDate);
            $toDate = new DateTime($leaveDetails->ToDate);
            $leaveDays = $fromDate->diff($toDate)->days + 1; // Include both FromDate and ToDate


            // Fetch employee position and salary
            $empid = $leaveDetails->empid;
            $sqlEmp = "SELECT tblemployees.FirstName, 
            tblemployees.LastName, 
            tblemployees.EmpId, 
            tblemployees.id AS EmployeeID, 
            tblemployees.Gender, 
            ...
            $queryEmp->bindParam(':leaveid', $did, PDO::PARAM_STR); // Use $did which holds leave ID
            $queryEmp->execute();
            $empDetails = $queryEmp->fetch(PDO::FETCH_OBJ);


            $basicSalary = $empDetails->salary;
            $deduction = 0;  // Default deduction value

            // Convert LeaveType to lowercase and trim any whitespace
            ...
            if ($leaveType === "earned leaves") {
                $deductionPercentagePerDay = 2;
            } elseif ($leaveType === "sick leaves") {
                $deductionPercentagePerDay = 3;
            } elseif ($leaveType === "rh (restricted leaves)") {
                $deductionPercentagePerDay = 4;
            } else {
                $deductionPercentagePerDay = 0; // No deduction for other leave types
            }
